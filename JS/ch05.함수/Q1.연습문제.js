...
function getQuadrant(x, y) {
  let quadrant = "";
  if (x > 0 && y > 0) quadrant = "1사분면";
  else if (x < 0 && y > 0) quadrant = "2사분면";
  else if (x < 0 && y < 0) quadrant = "3사분면";
  else if (x > 0 && y < 0) quadrant = "4사분면";
  else quadrant = "좌표축";
  return quadrant;
}
function answer2() {
...
function calculatePrize(dice1, dice2, dice3) {
  let prize = 0;
  if (dice1 == dice2 && dice2 == dice3) prize = 10000 + 1000 * dice1;
  else if (dice1 == dice2 || dice1 == dice3) prize = 1000 + 100 * dice1;
